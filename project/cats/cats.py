...
def accuracy(typed, source):
    ...
    if type_size == 0 :
        if source_size != 0:
            ...
            return 100.0
    count=0
    for i in range(min(type_size,source_size)):
        if typed_words[i] == source_words[i]:
            count+=1
    return count/type_size*100
    "*** YOUR CODE HERE ***"
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    minn_number = -1
    for i in range(len(word_list)):
        t = diff_function(typed_word,word_list[i],limit)
        if minn > t:
            minn = t
            minn_number = i
    if minn_number == -1 or minn > limit:
        return typed_word
    
    return word_list[minn_number]
...
def feline_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    """
    def correct(_typed,_source):
        if len(_typed) == 0:
            return len(_source)
        if len(_source) == 0:
            return len(_typed)
        
        if _typed[0]!=_source[0]:
            return correct(_typed[1:],_source[1:])+1
        else :
            return correct(_typed[1:],_source[1:])
    t = correct(typed,source)

    return limit + 1 if t > limit else t
    太多次调用了 只要发现就返回
    """
    def correct(_typed,_source,limit,count):
        if count > limit :
            return limit+1
        if len(_typed) == 0:
            count += len(_source)
        ...
        if _typed[0]!=_source[0]:
            count+=1
        return correct(_typed[1:],_source[1:],limit,count)

    return correct(typed,source,limit,0)
