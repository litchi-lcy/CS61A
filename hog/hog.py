...
def always_roll(n):
    ...
    def strategy(score,opponent_score):
        return n
    return strategy  #//注意注意 必须返回strategy
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    #if score >=goal or oppoent_score >=goal :
        #return False 
    """
    catch_up返回的也肯是相同的策略
    always_roll 是生成策略的工厂函数（比如 always_roll(3) 会返回一个具体的策略函数），而不是策略本身。
    因此 strategy 不可能等于 always_roll（因为 strategy 是工厂函数返回的结果）
    if strategy == catch_up :
        return False
    elif strategy == always_roll:
        return True
    """

    for score in range(goal):#从0-goal
        for oponent_goal in range(goal):
            if strategy(score,oponent_goal) != strategy(0,0):#这里的strategy就是对应的always_roll或者catch_up
                return False
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    result = sus_update(0,score,opponent_score)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    two_score = sus_points(score + roll_dice(2))
    if zero_score >= GOAL:
        return 0
    if one_score >= GOAL:
        return 1
