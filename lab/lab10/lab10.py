...
def floor_div(args):
    ...
    denominator = args.first
    numerator = args.rest
    while isinstance(numerator,Pair):
        denominator=denominator//(numerator.first)
...
def eval_and(expressions):
    ...
    if expressions is nil:
        return True
    result1=expressions.first
    result2=expressions.rest
    result1=calc_eval(result1)
    if  result1 is scheme_f:
        return False
    else :
        if result2 is nil:
            return result1
        else:
            return eval_and(result2)
